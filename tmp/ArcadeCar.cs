///
/// Copyright (c) 2018 Sergey Makeev
...
public class ArcadeCar : MonoBehaviour
{
    ...
    // m.baske: observables
    //
    public Vector3 Velocity => rb.velocity; // max 50
    public Vector3 AngularVelocity => rb.angularVelocity; // max 2.5
    public Vector3 Inclination => new Vector3(transform.right.y, transform.up.y, transform.forward.y);
    public Vector3 Anchor => transform.TransformPoint(anchor);
    public float SpeedKmH => GetSpeed() * 3.6f; // max 180
    public float SteerAngle => axles[0].steerAngle; // max 40
    public Quaternion Rotation => transform.rotation;

    private int roadLayer = 1 << 8;
    private Vector3 anchor = new Vector3(0, 0, 0);

    public bool WheelsAreOffRoad(int min)
    {
        int n = 0;
        for (int i = 0; i < 4; i++)
        {
            if (!Physics.Raycast(wheelTFs[i].position, Vector3.down, 10f, roadLayer))
            {
                n++;
            }
        }
    ...
    public Vector3 LocalizeDir(Vector3 dir)
    {
    ...
    public void Initialize()
    {
        ...
        wheelTFs = new Transform[4];
        for (int i = 0; i < 4; i++)
        {
            wheelTFs[i] = transform.GetChild(i + 1);
        }
    }
    ...
    float GetAccelerationForceMagnitude(AnimationCurve accCurve, float speedMetersPerSec, float dt)
    {
        ...
        float timeNow = BinSearch(accCurve, speedKmH, step, minTime, maxTime);
        // m.baske: replaced original brute-force with binary search.
        bool isResultFound = true; // TODO Should this always be true?
    ...
    float BinSearch(AnimationCurve curve, float value, float step, float tMin, float tMax)
    {
        float dMin = Mathf.Abs(value - curve.Evaluate(tMin));
        float dMax = Mathf.Abs(value - curve.Evaluate(tMax));
        bool isMin = dMin < dMax;
        float delta = tMax - tMin;
        float tMid = tMin + delta / 2f;

        return delta < step ? (isMin ? tMin : tMax) 
            : BinSearch(curve, value, step, isMin ? tMin : tMid, isMin ? tMid : tMax);
    }
    ...
    void UpdateInput(float v, float h, bool isHandBrakeNow)
    {
    ...
    public void StepUpdate(float acceleration, float steering, bool brake)
    {
        UpdateInput(acceleration, steering, brake);
